				...
				<div class="card mb-3">
					<div class="card-body">
						...
						foreach ($ticket_responses as $r) : ?>
							<div class="card mb-3">
								<div class="card-header <?= ($r->usuario_id == $ticket->usuario_id) ? "bg-secondary text-white" : "bg-info text-white" ?>">
									<?= ($r->usuario_id != $ticket->usuario_id) ? 'Atendente - ' : 'Cliente - ' ?><?= $r->nome ?>
								</div>
								<div class="card-body">
									<?= $r->descricao ?>
									<?php 
							...
							<?php if($ticket->escopo != 3) : ?>
								<div class="card">
									<div class="card-body">
										<form action="<?= base_url("tickets/ticket_reply_pro") ?>" method="POST" enctype="multipart/form-data">
						...
						<div class="col-4">
							<div class="card">
								<div class="card-body">
									...
									<p><b>Departamento: </b>
										<select class="form-control" onchange="change_departament($(this).val())">
											<?php foreach ($departamentos as $d) : ?>
												<option <?= ($ticket->departamento_id == $d->id) ? 'selected' : '' ?> value="<?= $d->id ?>"><?= $d->nome ?></option>
											<?php endforeach; ?>
										</select></p>
										<p><b>Status: </b>
											<select class="form-control" onchange="change_status($(this).val())">
												<?php foreach ($status as $s) : ?>
													<option <?= ($ticket->status_id == $s->id) ? 'selected' : '' ?> value="<?= $s->id ?>"><?= $s->nome ?></option>
													<?php endforeach; ?></select>
												</p>
												<p><b>Situação: </b><?= $ticket->escopo_nome ?></p>
												<br/>
						...
						<script type="text/javascript">
							...
							function change_departament(id) {
								event.preventDefault();
								swal(
									{title: 'Alterar Departamento',
									text: 'Desejar alterar o Departamento?',
									icon: 'warning',
									buttons: 
									[
									'Cancelar', 
								...
								.then(
									(willAlter) => {
										if (willAlter) {
											window.location.href = '<?= base_url("tickets/alter_departament_ticket/" . $ticket->ticketid); ?>/'+id;
										} else {
											return false;
										}
									});
							...
							function change_status(id) {
								...
								.then(
									(willAlter) => {
										if (willAlter) {
											window.location.href = '<?= base_url("tickets/alter_status_ticket/" . $ticket->ticketid); ?>/'+id;
										} else {
											return false;
										}
									});
